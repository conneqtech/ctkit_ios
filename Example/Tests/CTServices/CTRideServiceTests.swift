...
class CTRideServiceTests: XCTestCase {
    ...
    func testCreateRide() {
        self.stub(http(.post, uri: "/bike/743/ride"), json(Resolver().getJSONForResource(name: "ride"), status: 201))
        let startDate = Date() - 10000
        let subjectUnderTest = try! CTRideService().create(withBikeId: 743,
                                                     startDate: startDate,
                                                     endDate: Date(),
                                                     rideType: "ride.type.leisure",
                                                     name: "Vrijetijdsrit").toBlocking().first()!

        XCTAssertEqual(subjectUnderTest.name, "Vrijetijdsrit")
        XCTAssertEqual(subjectUnderTest.rideType, "ride.type.leisure")
    }
